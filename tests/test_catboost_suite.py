"""Unit tests for CatBoost trainer, early stopping, and triple gradient boosting benchmark."""
...
def test_catboost_training_and_early_stopping():
    """Assert CatBoost trains with early stopping and produces calibrated probabilities."""
    ...
    trainer = CatBoostTrainer(seed=42)
...
def test_catboost_vs_lgb_xgb_benchmark():
    """TRIPLE BENCHMARK RECOVERY TEST:

    Assert that CatBoost, LightGBM, and XGBoost all recover ground-truth signal above performance floor (PR AUC > 0.80).
    """
    ...
    cb_proba = cb_model.predict_proba(X_val_trans)[:, 1]
    lgb_proba = lgb_model.predict_proba(X_val_trans)[:, 1]
    xgb_proba = xgb_model.predict_proba(X_val_trans)[:, 1]

    cb_m = ModelEvaluator.compute_all_metrics(y_val.to_numpy(), cb_proba)
