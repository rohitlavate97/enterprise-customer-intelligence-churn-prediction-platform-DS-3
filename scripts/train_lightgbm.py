"""CLI script to train, tune, evaluate LightGBM and benchmark against XGBoost."""
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Train LightGBM and benchmark against XGBoost.")
    ...
    lgb_params = None
    if args.tune:
        lgb_params = lgb_trainer.tune_hyperparameters(X_tr_trans, y_tr.to_numpy(), cv_folds=3, n_iter=6)
    ...
    logger.info(f"\n==================== LIGHTGBM vs XGBOOST BENCHMARK ====================\n{benchmark_df.to_string(index=False)}")

    # Save LightGBM artifact
    lgb_artifact_path = settings.artifacts_dir / "lightgbm_model.joblib"
    joblib.dump(lgb_model, lgb_artifact_path)
    logger.info(f"Saved LightGBM model artifact to {lgb_artifact_path}")
